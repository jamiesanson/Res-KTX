...
class ResKtxPlugin: Plugin<Project> {
    ...
    private fun configureR2Generation(project: Project, variants: DomainObjectSet<out BaseVariant>) {
        variants.all { variant ->
            val outputDir = project.buildDir.resolve(
                "generated/source/k/${variant.dirName}")
            ...
            variant.outputs.all { output ->
                ...
                if (once.compareAndSet(false, true)) {
                    ...
                    val generate = project.tasks.create("generate${variant.name.capitalize()}K", KGenerator::class.java) {
